...
```
```
Capitalize every characters in acronyms if they have only 2 characters.
int ID;
```
```
Capitalize only first character in acronyms if they have more than 2 characters
int HttpCode;
```
```
...
```
```
Always add predefined FALLTHROUGH for switch case fall through. This will be replaced by [[fallthrough]] attribute coming in for C++17 later
switch (number)
{
  case 0:
    DoSomething();
...
```
```
When a class spans across multiple files, these files have a name that starts with the name of the class, followed by an underscore and a subsection name.
class RenderWorld;
...
Do not use auto unless it is for a iterator
